    ...
    public class ExceptionLecture
    {
        ...
        public void Part2OvertimeHours()
        {
            ...
            while (!isValidHours)
            {
                ...
                try // If anything goes wrong inside my curlies...
                {
                    hoursWorked = int.Parse(hoursString); // May throw a FormatException

                    // If we got here, hoursWorked is valid
                    isValidHours = true;
                }
                catch (FormatException ex) // Look for a catch here for that type of error
                {
                    Console.WriteLine("Oh snap! You typed something wrong. " + ex.Message);
        ...
        public void Part3CallStack()
        {
            ...
            */
            ...
            {
                Console.WriteLine("Someone tried to divide by 0. Please alert the authorities.");
            }
            /* NEVER do this
            catch (Exception ex)
            {
                // Swallows ANY exception in your application
            }
            catch // Same thing as catch(Exception)
            {
